...
*-----------------------------------------------------------------------------
    $PACKAGE MG.Config
      SUBROUTINE CONV.MG.PARAMETER.14.1
*
...
*NH*        IF YREC<COMPANY.CODE.POS> MATCHES "2A7N" THEN
         IF YREC<7>#'' THEN
         ...
         END ELSE
            ...
            IF FILE.TYPE#3 OR YID[1]=1 THEN        ; * live, unauthorised files + every 10th history record
               YREC<7>='YES'
            END ELSE                     ; * other history records
               YREC<7>='_'
            END
*
...
*
*NH*           X = 0
*NH*           LOOP X += 1 UNTIL CANCEL.FIELD<1,X> = ""
*NH*              POS = CANCEL.FIELD<1,X>
*NH*              NOF = CANCEL.FIELD<2,X>
*NH*              FOR Y = 1 TO NOF
*NH*                 DEL YREC<POS>
*NH*              NEXT Y
*NH*           REPEAT
...
*
*NH*           X = 0
*NH*           LOOP X += 1 UNTIL ADD.FIELD<1,X> = ''
*NH*              POS = ADD.FIELD<1,X>
*NH*              NOF = ADD.FIELD<2,X>
*NH*              FOR Y = 1 TO NOF
*NH*                 INS "" BEFORE YREC<POS>
*NH*              NEXT Y
*NH*           REPEAT
*
*

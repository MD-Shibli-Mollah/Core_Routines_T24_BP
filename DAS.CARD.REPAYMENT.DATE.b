* @ValidationCode : Mjo0MjA0NDk1MTc6Q3AxMjUyOjE1NjQ1Njk3NTQzNTA6c3JhdmlrdW1hcjotMTotMTowOi0xOnRydWU6Ti9BOkRFVl8yMDE5MDguMDotMTotMQ==
...
*-----------------------------------------------------------------------------
$PACKAGE CQ.Cards
SUBROUTINE DAS.CARD.REPAYMENT.DATE(THE.LIST, THE.ARGS, TABLE.SUFFIX)
*-----------------------------------------------------------------------------
* Data Access Servive for CARD.REPAYMENT.DATE
* Implements the query definition for all queries that can be used against
* the CARD.REPAYMENT.DATE table.
...
*-------------------------------------------------------
    ...
    $INSERT I_DAS.CARD.REPAYMENT.DATE.NOTES
    $INSERT I_DAS

BUILD.DATA:

    MY.TABLE = 'CARD.REPAYMENT.DATE' : TABLE.SUFFIX
*
    BEGIN CASE
        CASE MY.CMD = dasAllIds
*Records from CARD.REPAYMENT.DATE based on ID

        CASE MY.CMD = dasCardRepaymentDate$Date
            MY.FIELDS = '@ID'
            MY.OPERANDS = 'MATCHES'
            MY.DATA = THE.ARGS<1>
            MY.JOINS = ''

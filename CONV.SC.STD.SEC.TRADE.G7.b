...
*-----------------------------------------------------------------------------
    $PACKAGE SC.Config
      SUBROUTINE CONV.SC.STD.SEC.TRADE.G7
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.SC.STD.SEC.TRADE"       ; * File to be converted
      COMPANY.CODE.POS = 66              ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 59                ; * Position to add from. (New field number)
...
*
            ...
            LOOP X += 1 UNTIL ADD.FIELD<1,X> = ''
               ...
               NEXT Y
            REPEAT
* Clear down new reserved fields.
            YREC<36> = ''
            YREC<37> = ''
            YREC<46> = ''
*

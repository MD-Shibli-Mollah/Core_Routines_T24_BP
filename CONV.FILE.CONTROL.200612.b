...
*-----------------------------------------------------------------------------
    $PACKAGE IC.InterestAndCapitalisation
      SUBROUTINE CONV.FILE.CONTROL.200612
*-----------------------------------------------------------------------------
...
*
* Remove EOD.AC.VIOLATIONS.LIST from FILE.CONTROL if it exists
*
...
* look for DX.CO.NEXT.KEY only

      K.ID = 'EOD.AC.VIOLATIONS.LIST'
...
* open files and read records specific to each company
      ...
      F.FILE.CONTROL.TEMP = ''
      CALL OPF(ID,F.FILE.CONTROL.TEMP)
...
* read from FILE.CONTROL and get list of companies to be converted
      ...
      READ R.FILE.CONTROL FROM F.FILE.CONTROL,PGM.NAME ELSE
         CALL FATAL.ERROR('CONV.FILE.CONTROL.R7.200612')
      END

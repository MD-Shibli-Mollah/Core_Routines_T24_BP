...
*-----------------------------------------------------------------------------
    $PACKAGE IC.InterestAndCapitalisation
      SUBROUTINE CONV.GROUP.CAP.14.2
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.GROUP.CAPITALISATION"   ; * File to be converted
      COMPANY.CODE.POS = 11              ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 5                 ; * Position to add from. (New field number)
...
*
      ...
      ETEXT ="AC.RTN.WHY.PROGRAM.ABORTED.7"        ; * Used to update F.CONVERSION.PGMS
      CALL PGM.BREAK
*
*************************************************************************
   END

...
*-----------------------------------------------------------------------------
    $PACKAGE SC.SctDealerBook
      SUBROUTINE CONV.SC.ENT.TODAY.12.1.1
*
$INSERT I_COMMON
$INSERT I_EQUATE
$INSERT I_F.COMPANY
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.SC.ENT.TODAY"           ; * File to be converted
      ...
      READ R.FILE.CONTROL FROM F.FILE.CONTROL,ID ELSE
         ...
         GOTO FATAL.ERROR
      END
      MULTI.COMPANY.FILE = (R.FILE.CONTROL<EB.FILE.CONTROL.TYPE> NE 'INT')
      F.COMPANY = ''
...
*
      CANCEL.FIELD = ""
      CANCEL.FIELD<1,1> = ""             ; * Position to add from.
      CANCEL.FIELD<2,1> = ""             ; * Number of fields to add.
*

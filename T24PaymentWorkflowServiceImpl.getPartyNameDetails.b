* @ValidationCode : MjoxOTE2Mzc5Mjk2OkNwMTI1MjoxNTI3MTYzNzMxNjk4OmdtYW1hdGhhOjE6MDowOi0xOmZhbHNlOk4vQTpERVZfMjAxODA0LjIwMTgwMzIzLTAyMDE6Mjc6Mjc=
...
SUBROUTINE T24PaymentWorkflowServiceImpl.getPartyNameDetails(recApplication, currentFieldValue, responseDetails)
*-----------------------------------------------------------------------------------
*
* In/out parameters:
* recApplication - holds the POR.SUPPLEMENTARY.INFO table record details.
* currentFieldValue - holds the Name value for DEBTOR role of party Debit and ORDPTY role of party Credit.
*----------------------------------------------------------------------------------
...
*              To fetch Name field values from debit and credit table for particular role and Indicator.
*
*-----------------------------------------------------------------------------
...
*   Get multiple partyRole details from POR Supplementary table present for Debit and credit parties.
    ...
    GOSUB getDebitName  ;* To fetch Debit party Name
    GOSUB getCreditName ;* To fetch Credit party Name
    
RETURN
*-----------------------------------------------------------------------------
getDebitName:
*   To fetch Name from debit party where party role is equal to 'DEBTOR' and role Indicator equal to 'R',
*   If present then map it to the output parameter.
    ...
    LOCATE 'DEBTOR' IN partyRole<1,1> SETTING rolPosDebit THEN
        ...
        IF partyRoleIndicator EQ 'R' THEN
            currentFieldValue<1,-1> = recApplication<PP.PaymentWorkflowGUI.PorSupplementaryInfo.PorIdName,rolPosDebit>:'-':rolPosDebit
        END
    END
...
*--------------------------------------------------------------------------------
getCreditName:
*   To fetch Name from credit party where party role is equal to 'ORDPTY' and role Indicator equal to 'R',
*   If present then map it to the output parameter.
    partyRoleIndicator = ''
    LOCATE 'ORDPTY' IN partyRole<1,1> SETTING rolPosCredit THEN
        ...
        IF partyRoleIndicator EQ 'R' THEN
            currentFieldValue<1,-1> = recApplication<PP.PaymentWorkflowGUI.PorSupplementaryInfo.PorIdName,rolPosCredit>:'-':rolPosCredit
        END
    END

...
*-----------------------------------------------------------------------------
    $PACKAGE SC.SctModelling
      SUBROUTINE CONV.POLICY.PARAMETER.12.2
*
*
$INSERT I_COMMON
$INSERT I_EQUATE
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.POLICY.PARAMETER"       ; * File to be converted
      COMPANY.CODE.POS = 17              ; * Position of XX.CO.CODE in the file
      F.PGM.FILE = ''
...
*
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 5                 ; * Position to add from.
...
*
         ...
         IF YREC<COMPANY.CODE.POS> MATCHES "2A7N" ELSE
...
*
            ...
            REPEAT
*
* MAKE THE IDUSTRY CODE A MULTIVALUED FIELD
* ALSO THE PERCENTAGE FILED AND THE VARIANCE FIELD.
*
            IND.CODES = YREC<9>
            IND.PERC = YREC<10>
            IND.VARS = YREC<11>
            CONVERT "ü" TO "ý" IN IND.CODES
            CONVERT "ü" TO "ý" IN IND.PERC
            CONVERT "ü" TO "ý" IN IND.VARS
            YREC<9> = ""
            YREC<10> = ""
            YREC<11> = ""
*
            NO.CODES = DCOUNT(IND.CODES<1>,@VM)
            YY = 0
            FOR XX = 1 TO NO.CODES
               IF IND.CODES<1,XX> NE "" THEN
                  IND.CD = IND.CODES<1,XX>
                  LOCATE IND.CD IN YREC<9,1> SETTING POS ELSE
                     YY += 1
                     YREC<9,YY> = IND.CD
                     YREC<10,YY> = IND.PERC<1,XX>
                     YREC<11,YY> = IND.VARS<1,XX>
                  END
               END
            NEXT XX
*

...
*
*Subroutine to get the credit margin for Credit Interest calculation
*-----------------------------------------------------------------------------
* <Rating>339</Rating>
*-----------------------------------------------------------------------------
    $PACKAGE IC.ModelBank
    SUBROUTINE E.MB.IC.CR.MARGIN
*
...
********
    ...
    CNT11 = ""
    CNT11 = 401
    tmp=EB.Reports.getRRecord(); tmp<401>=""; EB.Reports.setRRecord(tmp)

    FOR RATE.CNT = 1 TO NO.OF.RATES
        IF OUT.DATA<1,RATE.CNT> EQ '' THEN
            tmp=EB.Reports.getRRecord(); tmp<401,-1>=0; EB.Reports.setRRecord(tmp)
        END ELSE
            tmp=EB.Reports.getRRecord(); tmp<401,-1>=OUT.DATA<1,RATE.CNT>; EB.Reports.setRRecord(tmp)
        END
    NEXT RATE.CNT
...
*
    BEGIN CASE
        CASE RET.DATA<IC.RC.CREDIT.RATE.TYPE,I> = 'F'
            CNT += 1
            OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.CREDIT.BAND.RATE, I>,'$',3)
            EB.Reports.setOData(OUT.DATA)

        CASE RET.DATA<IC.RC.CREDIT.RATE.TYPE,I> = 'B'
            IF RET.DATA<IC.RC.CREDIT.BAND.RATE, I, 1> NE '' THEN
                CNT += 1
                OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.CREDIT.BAND.RATE, I, 1>,'$',3)
            END
            NO.OF.RATES = DCOUNT(RET.DATA<IC.RC.CREDIT.BAND.RATE,I>, @SM)

            IF NO.OF.RATES = 1 THEN
                *OUT.DATA<CNT> := " over ":FMT(RET.DATA<IC.RC.CREDIT.BAND.UPTO, I, 1>,'L,0')
            END ELSE
                IF RET.DATA<IC.RC.CREDIT.BAND.UPTO, I, 1> NE '' THEN
                    *OUT.DATA<CNT> := " from 0 up to ":FMT(RET.DATA<IC.RC.CREDIT.BAND.UPTO, I, 1>,'L,0')
                END
                FOR J = 2 TO NO.OF.RATES
                    IF RET.DATA<IC.RC.CREDIT.BAND.RATE, I, J> NE '' THEN
                        CNT += 1
                        OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.CREDIT.BAND.RATE, I, J>,'$',3)
                        *OUT.DATA<CNT> := FMT(RET.DATA<IC.RC.CREDIT.BAND.UPTO, I, J-1>,'L,0')
                        IF J <> NO.OF.RATES THEN
                            *OUT.DATA<CNT> := " up to ":FMT(RET.DATA<IC.RC.CREDIT.BAND.UPTO, I, J>,'L,0')
                        END
                    END
                NEXT J
            END
        ...
        CASE RET.DATA<IC.RC.CREDIT.RATE.TYPE,I> = 'L'
            NO.OF.RATES = DCOUNT(RET.DATA<IC.RC.CREDIT.BAND.RATE,I>, @SM)
            IF NO.OF.RATES GT 1 THEN
                FOR J = 1 TO NO.OF.RATES
                    ...
                    IF J NE NO.OF.RATES THEN
                        OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.CREDIT.BAND.RATE, I, J>,'$',3)
                        *OUT.DATA<CNT> := FMT(RET.DATA<IC.RC.CREDIT.BAND.UPTO, I, J>,'L,0')
                    END ELSE
                        OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.CREDIT.BAND.RATE, I, J>,'$',3)
                        *OUT.DATA<CNT> := FMT(RET.DATA<IC.RC.CREDIT.BAND.UPTO, I, J-1>,'L,0')
                    END

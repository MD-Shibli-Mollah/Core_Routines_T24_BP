...
* <Rating>671</Rating>
*-----------------------------------------------------------------------------
      SUBROUTINE CONV.ENQ.REPORT.G7.1.02
...
*
* 15/01/97 - GB9700039
*            Correct the bug where by the Selection field (Old field 3)
*            is not expanded correctly.
*            There is no need to convert spaces to VM and then try and
*            find each component as this merges and loses informatioin
*            Instead take each Multi value of the old field and insert
*            the first part into the new field 3 and the second part
*            into the new file 4 (operand) and the remainder into the
*            new field 5 convert and separates to SM.
*            Also check record length for conversion run of 29 or 33
*            fields. Also comapny code check set field for company
*            code to 30.
*
*
*************************************************************************
*
* 28/11/96 - GB9601677
*            Remove all references to PGM.FILE as there is no record for it.
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.ENQUIRY.REPORT"         ; * File to be converted
      ORIGINAL.FILE = YFILE              ; * Store this it will changed
      COMPANY.CODE.POS = 30              ; * Position of new XX.CO.CODE in the file
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = "4"               ; * Position to add from. (New field number)
      ADD.FIELD<2,1> = "2"               ; * Number of fields to add.
*
*************************************************************************
*
...
***!            END
            END ELSE
               YNO.OF.FIELDS = DCOUNT(YREC,@FM)
               IF YNO.OF.FIELDS = 29 OR YNO.OF.FIELDS = 33 THEN
                  ALREADY.CONV += 1
               END ELSE
...
* GB9700039 Change Logic

      OLD.SELECT = TRIM(YREC<3>)
      YREC<3> = "" ; TOKEN = ""
      IF OLD.SELECT THEN
         EOS = FALSE
         LOOP
            REMOVE TOKEN FROM OLD.SELECT SETTING EOS
         WHILE TOKEN : EOS
            YREC<3,-1> = TOKEN[" ",1,1]
            YREC<4,-1> = TOKEN[" ",2,1]
            REST = TOKEN[" ",3,99]
            CONVERT SPACE(1) TO SM IN REST
            YREC<5,-1> = REST
         REPEAT
      END
      RETURN
...
*
      ...
      ETEXT ="EB.RTN.WHY.PROGRAM.ABORTED.4"        ; * Used to update F.CONVERSION.PGMS
      CALL PGM.BREAK
*
*************************************************************************
   END

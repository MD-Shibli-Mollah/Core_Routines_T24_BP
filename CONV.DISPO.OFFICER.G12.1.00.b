...
*-----------------------------------------------------------------------------
    $PACKAGE EB.OverrideProcessing
      SUBROUTINE CONV.DISPO.OFFICER.G12.1.00
*
...
*
* 05/12/02 =[2- GLOBUS_CI_10005261
*            Code changed to avoid unnecessary halts while running
*            Conversion Programs
*************************************************************************
INITIALISE:
*
      ...
      READ R.PGM.FILE FROM F.PGM.FILE,APPLICATION ELSE
         ID = APPLICATION
         YFILE = 'F.PGM.FILE'
         ETEXT = "EB.RTN.MISS.FILE.3":FM:APPLICATION:VM:YFILE          ; * GLOBUS_EN_10000035
         GOTO FATAL.ERROR
      END
      ...
      READ R.FILE.CONTROL FROM F.FILE.CONTROL,ID ELSE
         ...
         ETEXT = "EB.RTN.MISS.FILE.4":FM:ID:VM:YFILE         ; * GLOBUS_EN_10000035
         GOTO FATAL.ERROR
      END
...
* Open file to transfer data into
         ...
         OPEN DEST.FILE TO F.DEST.FILE ELSE
            ETEXT ="EB.RTN.UNABLE.OPEN.FILE.1":FM: DEST.FILE
            GOTO FATAL.ERROR
         END
...
*
            READ YREC FROM F.FILE, YID ELSE
               ETEXT = "EB.RTN.MISS.FILE.5":FM:YID:VM:YFILE  ; * GLOBUS_EN_10000035
               GOTO FATAL.ERROR
            END

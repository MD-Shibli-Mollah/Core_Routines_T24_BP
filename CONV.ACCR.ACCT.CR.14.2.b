...
*-----------------------------------------------------------------------------
    $PACKAGE IC.Config
      SUBROUTINE CONV.ACCR.ACCT.CR.14.2
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.ACCR.ACCT.CR"           ; * File to be converted
      ...
      YFILE.ADDS = ''
      YFILE.ADDS<1> = 'ACCR.ACCT.CR':VM:'ACCR.ACCT.DR':VM:'ACCR.ACCT.CR2':VM:'ACCR.ACCT.DR2'
      YFILE.ADDS<1> = YFILE.ADDS<1>:VM:'CORR.ACCT.CR':VM:'CORR.ACCT.DR':VM:'CORR.ACCT.CR2':VM:'CORR.ACCT.DR2'
*      YFILE.ADDS<1> = YFILE.ADDS<1>:VM:'GROUP.CONDS.CR':VM:'GROUP.CONDS.DR'
      YFILE.ADDS<1> = YFILE.ADDS<1>:VM:'INFO.ACCT.CR':VM:'INFO.ACCT.DR':VM:'INFO.ACCT.CR2':VM:'INFO.ACCT.DR2'
      YFILE.ADDS<1> = YFILE.ADDS<1>:VM:'STMT.ACCT.CR':VM:'STMT.ACCT.DR':VM:'STMT.ACCT.CR2':VM:'STMT.ACCT.DR2'
      YFILE.ADDS<1> = YFILE.ADDS<1>:VM:'ACCR.ACCT.CH':VM:'CORR.ACCT.CH':VM:'INFO.ACCT.CH':VM:'STMT.ACCT.CH'
      YFILE.ADDS<2> = 24:VM:76:VM:24:VM:23
      YFILE.ADDS<2> = YFILE.ADDS<2>:VM:25:VM:77:VM:25:VM:24
*      YFILE.ADDS<2> = YFILE.ADDS<2>:VM:15:VM:15
      YFILE.ADDS<2> = YFILE.ADDS<2>:VM:24:VM:76:VM:24:VM:23
      YFILE.ADDS<2> = YFILE.ADDS<2>:VM:25:VM:77:VM:25:VM:24
      YFILE.ADDS<2> = YFILE.ADDS<2>:VM:143:VM:130:VM:143:VM:144
      YNO.FILES = DCOUNT(YFILE.ADDS<1>,VM)
*
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 24                ; * Position to add from. (New field number)
...
*
      ...
      TEXT = ""
      FOR YFILE.COUNT = 1 TO YNO.FILES
         FILE.NAME = 'F':MNEMONIC:'.':YFILE.ADDS<1,YFILE.COUNT>
         ADD.FIELD<1,1> = YFILE.ADDS<2,YFILE.COUNT>
...
*         IF YREC<ADD.FIELD<1,1>> THEN
         IF DCOUNT(YREC,FM) => ADD.FIELD<1,1> THEN
...
*
            ...
            LOOP X += 1 UNTIL ADD.FIELD<1,X> = ''
               ...
               FOR Y = 1 TO NOF
                  INS "" BEFORE YREC<POS>
               NEXT Y
               YACC.KEY = FIELD(YID,'-',1)
               READ YACC.REC FROM F.ACCOUNT,YACC.KEY ELSE YACC.REC = ''
               IF YACC.REC<AC.INTEREST.LIQU.ACCT> THEN
                  YACC.KEY = YACC.REC<AC.INTEREST.LIQU.ACCT>
                  READ YACC.REC FROM F.ACCOUNT,YACC.KEY ELSE YACC.REC = ''
*                  YREC<ADD.FIELD<1,X>> = YACC.REC<AC.CURRENCY>
               END
               YREC<ADD.FIELD<1,X>> = YACC.REC<AC.CURRENCY>
            REPEAT
*

* @ValidationCode : Mjo3Njc3ODY1NzQ6Q3AxMjUyOjE2MTYxNjA2NTQ1MDg6dGhhbm1heWlrbDoxOjA6MDoxOmZhbHNlOk4vQTpERVZfMjAyMTAzLjA6MTI0OjEx
...
* @ValidationInfo : Coverage          : 11/124 (8.8%)
...
SUBROUTINE AA.DE.CONV.SELL.PART.COMMITMENT(InValue,HeaderRec,MvNo,OutValue,ErrorMsg)
*-----------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
*
* 03/02/21 - Enhancement: 4203218
*            Task : 4203221
*            Conversion routine for Sell Participant Commitment
*
...
*** To get the seller balances before share transfer read activity balances for amount transferred during share transfer and add this to current balance
    ...
    IF EffectiveDate GT EB.SystemTables.getToday() THEN 
        OutValue = CurAmt + UtlAmt + OvdAmt + ABS(FwdShareCurAmt) + ABS(FwdShareUtlAmt) + ABS(FwdShareOvdAmt)
    END ELSE
        OutValue = CurAmt + UtlAmt + OvdAmt + ABS(ShareCurAmount) + ABS(ShareUtlAmount) + ABS(ShareOvdAmount)
    END
    
    IF FIELD(OutValue,'.',2) ELSE       ;*If there i no decimal in commitment amount then add .00 to maintain uniformity with sell amount field in notice

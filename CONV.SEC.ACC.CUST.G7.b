...
*-----------------------------------------------------------------------------
    $PACKAGE SC.ScoPortfolioMaintenance
      SUBROUTINE CONV.SEC.ACC.CUST.G7
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.SEC.ACC.CUST"           ; * File to be converted
      ...
      END
      F.SEC.ACC.CUST = ''
      OPEN '','F.SEC.ACC.CUST' TO F.SEC.ACC.CUST ELSE
         ID = APPLICATION
         YFILE = 'F.SEC.ACC.CUST'
         GOTO FATAL.ERROR
      END
      F.COMPANY = ''
...
*
               YHOLD = ''
               READ YHOLD FROM F.SEC.ACC.CUST,YID THEN
                  IF YHOLD THEN
                     REC.CT = DCOUNT(YREC,FM)
                     FOR CT = 1 TO REC.CT
                        PORT.NO = YREC<CT>
                        LOCATE YREC<CT> IN YHOLD<1> BY 'AR' SETTING POSN ELSE
                           INS YREC<CT> BEFORE YHOLD<POSN>
                        END
                     NEXT CT
                     WRITE YHOLD TO F.SEC.ACC.CUST, YID
                  END ELSE
                     WRITE YREC TO F.SEC.ACC.CUST, YID
                  END
               END ELSE
                  WRITE YREC TO F.SEC.ACC.CUST, YID
               END

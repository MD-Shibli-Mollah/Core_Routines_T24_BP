...
* Version 5 06/06/01  GLOBUS Release No. 200511 31/10/05
*-----------------------------------------------------------------------------
* <Rating>832</Rating>
*-----------------------------------------------------------------------------
    $PACKAGE LC.Limits
      SUBROUTINE CONV.LC.LIMIT.TXNS.14.2
*
...
*
* 29/8/2002 - EN_10001035
* Converting from Error messages to Error Codes.
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.LIMIT.TXNS"             ; * File to be converted
      COMPANY.CODE.POS = "110"           ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
** file is being converted more than once.
      ADD.FIELD = ''
*      ADD.FIELD<1,1> = "94"              ; * Position to add from. (New field number)
*      ADD.FIELD<2,1> = "7"               ; * Number of fields to add.
*
*************************************************************************
*
...
*
            X = 0
            NUMBER.OF.RECS = COUNT(YREC,FM) + (YREC NE '')
            YREC2 = ''
            FOR I2 = 1 TO NUMBER.OF.RECS
               IF YREC<I2>[1,2] EQ 'TF' THEN
                  IF LEN(FIELD(YREC<I2>, '\', 1)) GT 12 THEN
                     DR.ID = FIELD(YREC<I2>, '\' ,1)
                     GOSUB PROCESS.DRAWINGS
                  END ELSE
                     LC.ID = FIELD(YREC<I2>, '\', 1)
...
PROCESS.DRAWINGS:
      ...
      DEL.FLAG = 0
      READ R.DRAWINGS FROM F.DRAWINGS, DR.ID ELSE R.DRAWINGS = ''
      IF R.DRAWINGS THEN
         ORG.DATE = FIELD(YREC<I2>,'\', 4)
         AMOUNT = FIELD(YREC<I2>, '\', 3)
         IF AMOUNT THEN

            MAT.DATE = R.DRAWINGS<TF.DR.MATURITY.REVIEW>
            IF NOT(MAT.DATE) THEN
               MAT.DATE = R.DRAWINGS<TF.DR.VALUE.DATE>
            END

*
            POS = INDEX(YREC<I2>, '\':ORG.DATE, 1)
            IF POS THEN
               YREC<I2>[POS+1,8] = MAT.DATE
            END ELSE
               DEL.FLAG = 1
            END
         END ELSE
...
PROCESS.LETTER:
      ...
      DEL.FLAG = 0
      READ R.LETTER FROM F.LETTER.OF.CREDIT, LC.ID ELSE R.LETTER = ''

      IF R.LETTER THEN
         ORG.DATE = FIELD(YREC<I2>,'\', 4)
         AMOUNT = FIELD(YREC<I2>, '\', 3)
         IF AMOUNT THEN
            MAT.DATE = R.LETTER<TF.LC.EXPIRY.DATE>
            POS = INDEX(YREC<I2>, '\':ORG.DATE, 1)
            IF POS THEN
               YREC<I2>[POS+1,8] = MAT.DATE
            END ELSE
...
*
      ...
      ETEXT ="LC.RTN.WHY.PROGRAM.ABORTED.9"        ; * Used to update F.CONVERSION.PGMS
      CALL PGM.BREAK
*
*************************************************************************
   END

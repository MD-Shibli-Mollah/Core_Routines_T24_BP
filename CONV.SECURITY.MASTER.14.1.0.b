...
* Version 5 16/05/01  GLOBUS Release No. 200511 31/10/05
*-----------------------------------------------------------------------------
* <Rating>896</Rating>
*-----------------------------------------------------------------------------
    $PACKAGE SC.ScoSecurityMasterMaintenance
      SUBROUTINE CONV.SECURITY.MASTER.14.1.0
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.SECURITY.MASTER"
      COMPANY.CODE.POS = 82              ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
*
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 69                ; * Position to add from. (New field number)
...
*
            ...
            IF MNEMONIC = 'BDG' THEN
               ...
               YREC<76,2> = ''
               MYREC = YREC<76>
               CONVERT VM TO '' IN MYREC
               IF MYREC = '' THEN YREC<76> = ''
            END
*

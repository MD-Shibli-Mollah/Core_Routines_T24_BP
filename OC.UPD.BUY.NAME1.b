* @ValidationCode : Mjo2MTkyNTg5MDc6Q3AxMjUyOjE1OTk0NzYyMzQ1ODQ6a2JoYXJhdGhyYWo6NTowOjA6MTpmYWxzZTpOL0E6REVWXzIwMjAwOC4yMDIwMDczMS0xMTUxOjU1OjU1
...
SUBROUTINE OC.UPD.BUY.NAME1(APPL.ID,APPL.REC,FIELD.POS,RET.VAL)
*-----------------------------------------------------------------------------
*<Routine description>
*
* Populate the value only when MIFID.REPORT.STATUS as "NEWT".
* Attached as a link routine in TX.TXN.BASE.MAPPING record to
* If NATIONAL.ID is populated in field BUYER.ID then fetch NAME.1 from CUSTOMER table for the counterparty
* Incoming parameters:
*
...
* Populate the value only when MIFID.REPORT.STATUS as "NEWT".
        IF MifidReportStatus EQ "NEWT" THEN
            GOSUB ASSIGN.NAME1.FROM.CUSTOMER ; *
        END
    END
RETURN
*** </region>
...
*** <region name= ASSIGN.NAME1.FROM.CUSTOMER>
ASSIGN.NAME1.FROM.CUSTOMER:
*** <desc> </desc>
    IF CustomerId THEN
        OcCusErr = ''
        ...
        IF NATIONAL.ID EQ NationalIdFromCustomer THEN ;*To Check if the NATIONAL.ID is present in the field BUYER.ID
            CusErr = ''
            CustomerRec = ST.Customer.Customer.CacheRead(CustomerId, CusErr)
            IF CustomerRec THEN
                Name1 = CustomerRec<ST.Customer.Customer.EbCusNameOne>
                IF Name1 THEN
                    RET.VAL = Name1
                END
            END

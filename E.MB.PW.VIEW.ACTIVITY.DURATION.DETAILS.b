* @ValidationCode : Mjo0OTE4MDg2MDg6Q3AxMjUyOjE1Nzg1NjQ0MTkyNDE6bWhpbmR1bWF0aHk6MTowOjA6MTpmYWxzZTpOL0E6REVWXzIwMjAwMS4yMDE5MTIyNC0xOTM1OjEwMDoxMDA=
...
SUBROUTINE E.MB.PW.VIEW.ACTIVITY.DURATION.DETAILS(YrDetails)
*-----------------------------------------------------------------------------
*
* This subroutine is attached as nofile to the enquiry PW.VIEW.ACTIVITY.DURATION.DETAILS.
* The routines calculates the time in days hours and minutes and returns the data to the
* enquiry along with activity trasaction details from the PW.ACTIVITY.TXN record for the current process.
*
...
CALCULATE.DURATION:
    ...
    YR.TIME.IN.HOURS.2 = FMT(FIELD((YR.TIME.IN.HOURS.1 * 60 + YR.TIME.IN.MINS.1)/60,'.',1),'R%2')
    YR.TIME.IN.MINS.2 = FMT(MOD((YR.TIME.IN.HOURS.1 * 60 + YR.TIME.IN.MINS.1),60),'R%2')
    IF YR.TIME.IN.HOURS.2 EQ '00' AND YR.TIME.IN.MINS.2 EQ '00' THEN
        YR.MONITOR.DURA = '00:01'
    END ELSE
        YR.MONITOR.DURA = YR.TIME.IN.HOURS.2 : ':' : YR.TIME.IN.MINS.2 ;* set the activity duration in hours and minutes
    END
...
CALC.DURATION:
    ...
    IF YR.NO.OF.DAYS EQ 1 THEN
        YR.MONITOR.DURA = YR.TIME.IN.HOURS.2:':':YR.TIME.IN.MINS.2 ;* set the activity duration in hours and minutes
    END ELSE
        YR.MONITOR.DURA = (YR.NO.OF.DAYS - 1):' D, ':YR.TIME.IN.HOURS.2:':':YR.TIME.IN.MINS.2 ;* set the activity duration in days, hours and minutes
    END

...
*-----------------------------------------------------------------------------
    ...
    SUBROUTINE CONV.TRN.CON.VALUE.DATE
*
*
*********************************************************
*
...
* CI_10028622 E
        IF R.COMPANY(EB.COM.MNEMONIC) EQ R.COMPANY(EB.COM.FINAN.FINAN.MNE) THEN
            GOSUB OPEN.FILES
            GOSUB PROCESS.TRN.CON.VALUE.DATE
        END
* CI_10028622 S
...
*
    FN.TRN.CON.VALUE.DATE = 'F.TRN.CON.VALUE.DATE'
    F.TRN.CON.VALUE.DATE = ''
    CALL OPF(FN.TRN.CON.VALUE.DATE,F.TRN.CON.VALUE.DATE)
*
    RETURN

******************************************************************
PROCESS.TRN.CON.VALUE.DATE:
*
* Select list
*
        SP.LIST = ''
        SELECT.SP = 'SSELECT ':FN.TRN.CON.VALUE.DATE
        CALL EB.READLIST(SELECT.SP,SP.LIST,'','','')

*
* Main loop (process TRN.CON.VALUE.DATE)
*
        LOOP
...
*
        WHILE SP.CODE DO
*
            CALL F.READ(FN.TRN.CON.VALUE.DATE,SP.CODE,R.FILE,F.TRN.CON.VALUE.DATE,ER)
            IF ER THEN
                E = 'RECORD & NOT FOUND ON FILE & ':FM:SP.CODE:VM:"F.TRN.CON.VALUE.DATE"
                GOTO FATAL.ERROR
            END
*
            SP.CODE.NEW = SP.CODE:'.'

            CALL F.WRITE(FN.TRN.CON.VALUE.DATE,SP.CODE.NEW,R.FILE)    ;*save new record
*
            CALL F.DELETE(FN.TRN.CON.VALUE.DATE,SP.CODE)    ;*delete old record
*
...
FATAL.ERROR:
    ...
    TEXT = E
    CALL FATAL.ERROR('CONV.TRN.CON.VALUE.DATE')

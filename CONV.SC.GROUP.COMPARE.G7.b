...
*-----------------------------------------------------------------------------
    $PACKAGE SC.SctModelling
      SUBROUTINE CONV.SC.GROUP.COMPARE.G7
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.SC.GROUP.COMPARE"       ; * File to be converted
      ORIGINAL.FILE = YFILE              ; * Store this it will changed
      COMPANY.CODE.POS = "24"            ; * Position of new XX.CO.CODE in the file
      INPUTTER.POS = "21"                ; * Position of INPUTTER to store conversion id
      F.PGM.FILE = ''
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 14                ; * Position to add from. (New field number)

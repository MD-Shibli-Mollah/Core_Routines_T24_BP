...
*-----------------------------------------------------------------------------
    $PACKAGE SC.Config
      SUBROUTINE CONV.CUST.SECURITY.14.2
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.CUSTOMER.SECURITY"      ; * File to be converted
      COMPANY.CODE.POS = "31"            ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
*
            ...
            LOOP X += 1 UNTIL ADD.FIELD<1,X> = ''
               ...
               NEXT Y
            REPEAT
            LOCATE 'DEPOSITORY' IN YREC<1> SETTING POS ELSE POS = 0
            IF POS THEN
               YREC<23> = 'Y'
            END
*

* @ValidationCode : MjotNDA5NTU3NDUyOkNwMTI1MjoxNjEwMzk3NzIxOTYyOnJkZWVwaWdhOjY6MDowOjE6ZmFsc2U6Ti9BOkRFVl8yMDIwMTIuMjAyMDExMjgtMDYzMDo0NDo0NA==
...
SUBROUTINE SCDX.TRS.UPD.SELL.DEC.MKR.NAME2(TXN.ID,TXN.REC,TXN.DATA,RET.VAL)
*-----------------------------------------------------------------------------
...
* Outgoing parameters:
**********************
* RET.VAL  -  If Customer is Seller, then RET.VAL will be Last Name of Decision Maker
*
*-----------------------------------------------------------------------------
...
*** <desc>Process to determine the customer side is Seller when Decision Maker inputted in txn for reporting purpose </desc>
    ...
    BEGIN CASE
* Security Txn:
* If the customer mentioned in CUSTOMER.NO is Seller, then Last name of the customer defined in the field DECISION.MKR.ID will be returned
...
* Derivative Txn:
* If PRI.CUST.TYPE is Customer and PRI.BUY.SELL is SELL, then shall populate this field with the Last name of the customer defined in the field DECISION.MKR.ID.
* If SEC.CUST.TYPE is Customer and SEC.BUY.SELL is SELL, then shall populate this field with the Last name of the customer defined in the field DECISION.MKR.ID.

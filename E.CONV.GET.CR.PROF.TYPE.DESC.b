...
*-----------------------------------------------------------------------------
    ...
    SUBROUTINE E.CONV.GET.CR.PROF.TYPE.DESC
*-----------------------------------------------------------------------------
* Conversion routine to display description from CR.PROFILE.TYPE
...
*-----------------------------------------------------------------------------
    IF ID.DATA THEN
        ...
        R.CR.PROFILE.TYPE = CR.Analytical.ProfileType.Read(ID.DATA, CR.ERR)
        EB.Reports.setOData(R.CR.PROFILE.TYPE<1,EB.SystemTables.getLngg()>)
    END
    END
    RETURN
*-----------------------------------------------------------------------------

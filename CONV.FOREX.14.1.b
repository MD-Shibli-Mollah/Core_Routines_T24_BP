...
*
    ...
    NEXT X
    CLS := @(0,4)
    YFILE = "F.FOREX"         ;* File to be converted
    COMPANY.CODE.POS = 118    ;* Position of XX.CO.CODE in the file
    F.PGM.FILE = ''
    ...
    READ R.PGM.FILE FROM F.PGM.FILE,APPLICATION ELSE
        ID = APPLICATION
        YFILE = 'F.PGM.FILE'
* GOTO is replaced by GOSUB while calling FATAL.ERROR.
        GOSUB FATAL.ERROR
    END
...
*
    ADD.FIELD = ''
    ADD.FIELD<1,1> = 100      ;* Position to add from.
    ADD.FIELD<2,1> = 6        ;* Number of fields to add.
    ADD.FIELD<1,2> = 49       ;* Position to add from.
...
FIND.POSITION:
    ...
    V$COUNT += 1    ;* Count sucessfull conversions.
*

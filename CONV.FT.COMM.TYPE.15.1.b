...
*-----------------------------------------------------------------------------
    $PACKAGE FT.Contract
      SUBROUTINE CONV.FT.COMM.TYPE.15.1
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.FT.COMMISSION.TYPE"     ; * File to be converted
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = "31"              ; * Position to add from. (New field number)
      ADD.FIELD<2,1> = "5"               ; * Number of fields to add.
      ADD.FIELD<1,2> = "30"              ; * Position to add from. (New field number)

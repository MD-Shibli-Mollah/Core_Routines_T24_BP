...
* <Rating>340</Rating>
*-----------------------------------------------------------------------------
*Routine to get the basic credit interest of an account
*-----------------------------------------------------------------------------
    $PACKAGE IC.ModelBank
    SUBROUTINE E.MB.IC.CR.BASIC
*
************************************************************************
...
*
* 17/08/10       -   Defect 76621 / Task 77531
*                    Interest rate aligned properly, when the interest rate
*                    is null for the prior (ACI/GDI) record.
*
...
*=======
    ...
    CNT11 = ""
    CNT11 = 400
    tmp=EB.Reports.getRRecord(); tmp<400>=""; EB.Reports.setRRecord(tmp)
    FOR RATE.CNT = 1 TO NO.OF.RATES
        IF OUT.DATA<1,RATE.CNT> EQ '' THEN
            tmp=EB.Reports.getRRecord(); tmp<400,-1>=0; EB.Reports.setRRecord(tmp)
        END ELSE
            tmp=EB.Reports.getRRecord(); tmp<400,-1>=OUT.DATA<1,RATE.CNT>; EB.Reports.setRRecord(tmp)
        END
    NEXT RATE.CNT
*
    RETURN
            ...
            END ELSE
                ...
                FOR J = 2 TO NO.OF.RATES
                    IF RET.DATA<IC.RC.CREDIT.BAND.RATE, I, J> NE '' THEN
                        CNT += 1
                        OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.CREDIT.BAND.RATE, I, J>,'$',2)

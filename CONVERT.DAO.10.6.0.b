...
*************************************************************************
*
      YFILE = "F.DEPT.ACCT.OFFICER"
      Y1ST.FIELD.CANCEL = "" ; YLAST.FIELD.CANCEL = ""
*
      ADD.FIELD = ''
*
      ADD.FIELD<1,1> = 6                 ; * POSITION TO ADD FROM
...
*
         IF YREC<EB.DAO.CO.CODE> = ID.COMPANY THEN
            PRINT @(1,8):FMT("CONVERSION ALREADY DONE ":YID, "60L"):
            GOTO NEXT.READNEXT
         END
*
         IF YFILE[4] = "$HIS" THEN       ; * History file
            IF YREC<EB.DAO.CO.CODE> = "_" AND NUM(YREC<EB.DAO.CURR.NO>) AND YREC<EB.DAO.AUTHORISER>["_",2,1] # "" THEN
               PRINT @(1,8):FMT("CONVERSION ALREADY DONE ":YID, "60L"):
               GOTO NEXT.READNEXT
            END
         END

...
* Version 6 15/05/01  GLOBUS Release No. 200511 31/10/05
    ...
    $PACKAGE FT.Clearing
      SUBROUTINE CONV.FT.LOC.CLEARING.12.2.2
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.FT.LOCAL.CLEARING"      ; * File to be converted
      COMPANY.CODE.POS = 38              ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
*
      CANCEL.FIELD = ""
      CANCEL.FIELD<1,1> = 24             ; * Position to cancel from.
      CANCEL.FIELD<2,1> = 1              ; * Number of fields to cancel.
*
...
*
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 23                ; * Position to add from. (New field number)
...
*
            ...
            REPEAT
*
** Add the correct APPLICATION for the local reference item
*
            BEGIN CASE
               CASE YREC<1> = "SIC"      ; * Swiss
                  APP = "FUNDS.TRANSFER"
               CASE YREC<1> = "BGC"      ; * Dutch
                  APP = "ACCOUNT"
               CASE YREC<1> = "SCC"
                  APP = "FUNDS.TRANSFER"
               CASE 1
                  APP = ""
            END CASE
            IF APP THEN                  ; * Add the fields
               NO.LCRS = DCOUNT(YREC<22>,VM)
               FOR NLC = 1 TO NO.LCRS
                  YREC<23,NLC> = APP
               NEXT NLC
            END
*

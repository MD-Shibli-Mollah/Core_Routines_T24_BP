...
*-----------------------------------------------------------------------------
    $PACKAGE LC.Config
      SUBROUTINE CONV.LC.PARAMETER.G5.0.03
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.LC.PARAMETERS"          ; * File to be converted
      COMPANY.CODE.POS = "72"            ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = "58"              ; * Position to add from. (New field number)
...
*
      ...
      ETEXT ="LC.RTN.WHY.PROGRAM.ABORTED.11"       ; * Used to update F.CONVERSION.PGMS
      CALL PGM.BREAK
*
*************************************************************************
   END

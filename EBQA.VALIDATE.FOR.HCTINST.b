* @ValidationCode : MjoxNjkyMDI0MTEzOmNwMTI1MjoxNTcxOTk1NjY2Njc4OmdtYW1hdGhhOjM6MDowOjE6ZmFsc2U6Ti9BOkRFVl8yMDE5MDguMjAxOTA3MDUtMDI0Nzo0NTo0NQ==
* @ValidationInfo : Timestamp         : 25 Oct 2019 14:57:46
...
*---------------------------------------------------------------------------------------------------------------
$PACKAGE PPIHCT.Foundation
SUBROUTINE EBQA.VALIDATE.FOR.HCTINST
*-------------------------------------------------------------------------------------------------------------
*
* This is a new api to perform clearing specific validation for HCTINST payments.
...
*              New clearing specific api to validate outgoing camt.056/camt.09 messages for HCTINST payments
*
*-------------------------------------------------------------------------------------------------------------
    ...
    BEGIN CASE
        CASE iISORejectReasonCode EQ 'LEGL'
            IF rejAddtlInfCount GT 2 THEN
                ...
                EB.SystemTables.setEtext("Occurrences of RejectAddlInfo cannot be more than 2 for given Reason Code")
                EB.ErrorProcessing.StoreEndError()
            END
    
        CASE 1
            IF (iRejectAddlInfo NE '') AND (iISORejectReasonCode NE '') THEN
                EB.SystemTables.setAf(DE.Messaging.EbQueriesAnswers.EbQaRejectAddlInfo)
                EB.SystemTables.setAv(rejAddtlInfCount)

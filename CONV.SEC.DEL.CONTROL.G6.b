...
*-----------------------------------------------------------------------------
    $PACKAGE SC.SctSettlement
      SUBROUTINE CONV.SEC.DEL.CONTROL.G6
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.SEC.DEL.CONTROL"        ; * File to be converted
      COMPANY.CODE.POS = 28              ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
*
            ...
            LOOP X += 1 UNTIL ADD.FIELD<1,X> = ''
               ...
               NEXT Y
               IF POS = '14' THEN
                  YREC<POS> = YREC<13> - SUM(YREC<18>)
               END
               IF POS = '13' THEN
                  YREC<POS> = YREC<12> - SUM(YREC<16>)
               END
            REPEAT
*

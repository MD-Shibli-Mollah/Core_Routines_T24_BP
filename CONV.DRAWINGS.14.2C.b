...
*-----------------------------------------------------------------------------
    $PACKAGE LC.Contract
      SUBROUTINE CONV.DRAWINGS.14.2C
*
...
*
            IF YREC<7> THEN
               IF YREC<7> LT TODAY THEN
                  YREC<66> = YREC<7>     ; * To stop payment advices when mat.
               END
            END
            IF NOT(YREC<95>) THEN
               READ LCREC FROM F.LETTER.OF.CREDIT, YID[1,12] ELSE LCREC = ''
               IF LCREC THEN
                  YREC<95> = LCREC<2>
                  YREC<96> = LCREC<18>
               END
...
*
      ...
      ETEXT ="LC.RTN.WHY.PROGRAM.ABORTED.4"        ; * Used to update F.CONVERSION.PGMS
      CALL PGM.BREAK
*
*************************************************************************
   END

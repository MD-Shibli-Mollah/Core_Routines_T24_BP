...
*-----------------------------------------------------------------------------
    ...
    SUBROUTINE CONV.SC.FEE.GROUP.HIST.G15.0.FILE
*-----------------------------------------------------------------------------
...
*--------------
    F.SC.FEE.GROUP = ''
    OPEN 'F.SC.FEE.GROUP.HIST' TO F.SC.FEE.GROUP ELSE
        ETEXT  = 'CANNOT OPEN FILE F.SC.FEE.GROUP.HIST'
    END
...
INITIALISE:
    ...
    FILE.TO.CONVERT = 'SC.FEE.GROUP.HIST'
    SAM.FIELD = 0
...
* use direct SELECT, as EB.READLIST inturn calls OPF where it tries to open FBNK...
    ...
    IF YERR = '' THEN
        * Check that the record did exist!
        SEC.ACC.MASTER.ID = FIELD(SOURCE.ID,"*",1)
        SAM.ERR = ''

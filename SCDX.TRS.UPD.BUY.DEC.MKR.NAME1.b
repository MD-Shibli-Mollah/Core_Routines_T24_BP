* @ValidationCode : MjotMTkzNzMzMTIzODpDcDEyNTI6MTYxMDM3MzkzMjg5MDpyZGVlcGlnYTo1OjA6MDoxOmZhbHNlOk4vQTpERVZfMjAyMDEyLjIwMjAxMTI4LTA2MzA6NDI6NDE=
...
SUBROUTINE SCDX.TRS.UPD.BUY.DEC.MKR.NAME1(TXN.ID,TXN.REC,TXN.DATA,RET.VAL)
*-----------------------------------------------------------------------------
...
* Outgoing parameters:
**********************
* RET.VAL  -  If Customer is Buyer, then RET.VAL will be First Name of Decision Maker
*-----------------------------------------------------------------------------
...
*** <desc>Process to determine the customer side is Buyer when Decision Maker inputted in txn for reporting purpose </desc>
    ...
    BEGIN CASE
* Security Txn:
* If the customer mentioned in CUSTOMER.NO is Buyer, then first name of the customer defined in the field DECISION.MKR.ID will be returned
...
* Derivative Txn:
* If PRI.CUST.TYPE is Customer and PRI.BUY.SELL is BUY, then shall populate this field with the first name of the customer defined in the field DECISION.MKR.ID.
* If SEC.CUST.TYPE is Customer and SEC.BUY.SELL is BUY, then shall populate this field with the first name of the customer defined in the field DECISION.MKR.ID.

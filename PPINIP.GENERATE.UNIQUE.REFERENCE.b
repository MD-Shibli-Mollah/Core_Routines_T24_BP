* @ValidationCode : MjotMTA3MjcwNzAwNzpDcDEyNTI6MTYxODQ4MDIzMTkzMjpzdHV0aS5zaW5naDo1OjA6MDoxOmZhbHNlOk4vQTpERVZfMjAyMTAzLjIwMjEwMzAxLTA1NTY6NjQ6NTU=
...
* @ValidationInfo : Coverage          : 55/64 (85.9%)
...
SUBROUTINE PPINIP.GENERATE.UNIQUE.REFERENCE(iLockingId,iAgentDigits,iRandomDigitsLen,iReserved,oUniqueReference,oReserved)
*-----------------------------------------------------------------------------
...
*** <region name= Process>
Process:
*** <desc>Generate  10 digit unique reference number </desc>
...
*So, Changing Read to Readu to lock the record
    ...
    IF rLocking EQ '' THEN  ;* if there is no record, write a new one with the sequence number and today date
        ...
        rLocking<2> = SeqNo
    END ELSE
        IF rLocking<2> EQ '9999999999' THEN  ;* If value reaches 9999999999 then start the sequence again
            RSuffix = ''
            SeqNo = '1' ;* sequence number
            rLocking<2> = SeqNo
        END ELSE

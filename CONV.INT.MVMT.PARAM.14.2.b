* @ValidationCode : MjotMTYxMTkyNTI4MDpjcDEyNTI6MTU2NzU5NjE4MjI2NzpzcmF2aWt1bWFyOi0xOi0xOjA6LTE6ZmFsc2U6Ti9BOkRFVl8yMDE5MDguMTotMTotMQ==
...
*
    ...
    NEXT X
    CLS := @(0,4)
    YFILE = "F.INT.MOVEMENT.PARAM"
    COMPANY.CODE.POS = "27"            ; * Position of new XX.CO.CODE in the file
    F.PGM.FILE = ''
...
** file is being converted more than once.
    ADD.FIELD = ''
    ADD.FIELD<1,1> = 10                ; * Position to add from. (New field number)
    ADD.FIELD<2,1> = 2                 ; * Number of fields to add.
    ADD.FIELD<1,2> = 9                 ; * Position to add from. (New field number)
    ADD.FIELD<2,2> = 2                 ; * Number of fields to add.
*
*************************************************************************
*
...
*
            ...
            REPEAT
*
** For each code specified add the default entry type to
** be STMT
*
            ENT.POS = 8                  ; * DR.INT.CODES
            NO.CODES = DCOUNT(YREC<ENT.POS>,VM)    ; * No Values
            FOR ET = 1 TO NO.CODES
                SMC = DCOUNT(YREC<ENT.POS,ET>,SM)
                FOR ST = 1 TO SMC
                    IF YREC<ENT.POS,ET,ST> THEN
                        YREC<ENT.POS+1,ET,ST> = "STMT"
                    END
                NEXT ST
            NEXT ET
*
            ENT.POS = 11                 ; * CR.INT.CODES
            NO.CODES = DCOUNT(YREC<ENT.POS>,VM)    ; * No Values
            FOR ET = 1 TO NO.CODES
                SMC = DCOUNT(YREC<ENT.POS,ET>,SM)
                FOR ST = 1 TO SMC
...
*
    ...
    ETEXT ="RE.RTN.WHY.PROGRAM.ABORTED.2"        ; * Used to update F.CONVERSION.PGMS
    CALL PGM.BREAK
*
*************************************************************************
END

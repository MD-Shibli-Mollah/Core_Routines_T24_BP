...
*-----------------------------------------------------------------------------
    $PACKAGE SC.SctDealerBook
      SUBROUTINE CONV.SC.TRADING.POSITION.G6115
*
...
*
      ...
      END
*
* no conversion if previous release LT G6.1.11 or GE G6.1.15
*
      ...
      BEGIN CASE
         CASE PREVIOUS.RELEASE[1,5] NE 'G6.1.'
            DONT.RUN = 1
         CASE VER.NO LT 11 OR VER.NO GE 15
            DONT.RUN = 1
         CASE 1
            DONT.RUN = 0
      END CASE
*
      IF DONT.RUN THEN
         SUMMARY.REPORT = R.USER<EB.USE.USER.NAME>:' ':TIMEDATE()      ; * Summa ry of files & number of records converted.
...
** file is being converted more than once.
      ...
      ADD.FIELD<1,2> = 23                ; * Position to add from. (New field number)

* @ValidationCode : MjoyMTQ2OTUyNTkwOkNwMTI1MjoxNTk3MDczNTQyMTE0OnJldmF0aHlyYW1lc2g6NTowOjA6MTpmYWxzZTpOL0E6REVWXzIwMjAwNi4yMDIwMDUyNy0wNDM1OjIyMDoyMTU=
* @ValidationInfo : Timestamp         : 10 Aug 2020 21:02:22
...
SUBROUTINE SC.OFS.SETR.004.MAPPING
*-----------------------------------------------------------------------------
* This routine performs mapping of SETR.004 Inward message.
*-----------------------------------------------------------------------------
...
INITIALISATION:
    ...
    R.INWARD = ''
    R.SWIFT = '' ;*Not used
    MESSAGE.TYPE = 'SETR004' ;*MESSAGE.TYPE field
    R.DE.MESSAGE = ''
    OFS.MESSAGE = ''
    OFS.KEY = ''
    YERR=''

    GOSUB MX.PROCESS          ;*Convert the SETR004 MX message
    ...
    IF NOT(SWIFT.SRC.VER) THEN
        GOSUB INELIG.CUSTOMER ; *Raise this error in case of ineligible customer
    END
...
MAIN.PROCESSING:
    ...
    IF NOT(R.SEC.OPEN.ORDER) THEN
        ...
        ORD.DET<1> = 'SEL'        ;* TransactionCode
        ORD.DET<2> = RECORD<2>    ;* Order DateTime
        ORD.DET<3> = RECORD<7>    ;* ISIN
        ORD.DET<4> = RECORD<4>    ;* Portfolio/CustodyPort
        ORD.DET<5> = RECORD<5>    ;* BIC
        ORD.DET<6> = RECORD<9>    ;* Nominals
        ORD.DET<7> = RECORD<15>   ;* SettlementCcy
        ORD.DET<8> = RECORD<12>   ;* CuDepository
        ORD.DET<9> = RECORD<10>   ;* BrokDelivInstr
        ORD.DET<10> = RECORD<3> ;* Mapping ExpiryDate from Message
...
* if there is an error while transforming then log it in exception
        EB.ErrorProcessing.ExceptionLog("S","SEC.OPEN.ORDER","SC.OFS.SETR.004.MAPPING","SECURITIES",'',RESULT.XML,'SEC.OPEN.ORDER',tmp.R.KEY,'1',RESULT.XML,'')
    END ELSE
...
;* Write the SOO ID to SC.ORD.CUST.REF.WRK file with ID as ORDER.REF. Which will be used to find the SOO id when the customer sends further messages with ORDER.REF
        SC.SctOrderCapture.OrdCustRefWrK.Write(RECORD<6>, INSTR.ID)
    END
...
* Updating the Message details to the log file.
    MSG.DET = ''
    MSG.DET<1> = RECORD<6>      ;* Inward Reference
    MSG.DET<2> = 'SETR004'      ;* MessageType
    MSG.DET<3> = 'INWARD'       ;*MessageInOut
    MSG.DET<4> = 'NEWM'         ;* MessageFunction
    MSG.DET<5> = RECORD<4>      ;* CustodyPort/SAM
    MSG.DET<6> = RECORD<5>      ;* BIC
    MSG.DET<7> = RECORD<17>     ;* IBAN
...
*For Ineligible customer SETR016 will be sent out with Error reason
        ...
        OUT.DETS<11> = 'B-':ORD.DET<5> ;*Outward needs to be send out to the Customer
        OUT.DETS<12> = 'INELIGIBLE CUSTOMER' ;*reason to be sent
...
*when the order is created without any err, outward will be generated with the order status
            ...
            OUT.DETS<4> = R.SOO.REC<SC.SctOrderCapture.SecOpenOrder.ScSooSecurityNo> ;*Security
            OUT.DETS<5> = R.SOO.REC<SC.SctOrderCapture.SecOpenOrder.ScSooStockExchange> ;*StockExchange

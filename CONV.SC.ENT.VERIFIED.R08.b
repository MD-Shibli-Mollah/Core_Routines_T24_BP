...
*-----------------------------------------------------------------------------
    ...
    SUBROUTINE CONV.SC.ENT.VERIFIED.R08
* Conversion Routine for the removal of SC.ENT.VERIFIED records.
...
*----------------------
    ...
    FN.SC.ENT.VERIFIED = 'F.SC.ENT.VERIFIED'
    F.SC.ENT.VERIFIED = ''
    CALL OPF(FN.SC.ENT.VERIFIED, F.SC.ENT.VERIFIED)

    ENT.VERIFIED.LIST = dasAllIds
    THE.ARGS = ''
    TABLE.SUFFIX = ''
    CALL DAS('SC.ENT.VERIFIED', ENT.VERIFIED.LIST, THE.ARGS, TABLE.SUFFIX)

    IF ENT.VERIFIED.LIST THEN
        LOOP
            REMOVE LIST.REF FROM ENT.VERIFIED.LIST SETTING MORE.ENT
        WHILE LIST.REF:MORE.ENT
            DELETE F.SC.ENT.VERIFIED,LIST.REF
        REPEAT
    END

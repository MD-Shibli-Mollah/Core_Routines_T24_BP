...
*-----------------------------------------------------------------------------
    $PACKAGE SC.SctPositionTransfer
      SUBROUTINE CONV.POSITION.TRANS.G6.1
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.POSITION.TRANSFER"      ; * File to be converted
...
** file is being converted more than once.
      ADD.FIELD = ''
* Introduce new field EXCHANGE.DATE.
      ADD.FIELD<1,1> = 20                ; * Position to add from. (New field number)
      ADD.FIELD<2,1> = 1                 ; * Number of fields to add.
*
      ADD.FIELD<1,2> = 10                ; * Position to add from. (New field number)

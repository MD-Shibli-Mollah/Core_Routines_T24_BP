...
*
    FN.POS.CON.SEC = 'F.POS.CON.SEC'
    F.POS.CON.SEC = ''
    CALL OPF(FN.POS.CON.SEC,F.POS.CON.SEC)
*
    RETURN
...
*
    ...
    SEL.CMD = "SELECT ":FN.POS.CON.SEC
    CALL EB.READLIST(SEL.CMD,SEL.LIST,"","","")
*
    LOOP
        REMOVE PORT FROM SEL.LIST SETTING MORE
    WHILE PORT:MORE
        CALL F.READ(FN.POS.CON.SEC,PORT,R.FILE,F.POS.CON.SEC,ER)
        IF ER THEN
            E = 'RECORD & NOT FOUND ON FILE & ':FM:PORT:VM:"F.POS.CON.SEC"
            GOTO FATAL.ERROR
        END
*
...
*
        FOR I = 1 TO REC.NB   ;*process each line
            R.FILE<I> = R.FILE<I>:'.'
        NEXT I
        CALL F.WRITE(FN.POS.CON.SEC,PORT,R.FILE)  ;*save the record
...
FATAL.ERROR:
    ...
    TEXT = E
    CALL FATAL.ERROR('CONV.POS.CON.SEC')

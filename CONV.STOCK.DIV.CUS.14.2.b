...
*-----------------------------------------------------------------------------
    $PACKAGE SC.SccClassicCA
      SUBROUTINE CONV.STOCK.DIV.CUS.14.2
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.STOCK.DIV.CUS"          ; * File to be converted
      COMPANY.CODE.POS = "48"            ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,2> = "14"              ; * Position to add from. (New field number)
      ADD.FIELD<2,2> = "1"               ; * Number of fields to add.
      ADD.FIELD<1,1> = "35"              ; * Position to add from. (New field number)

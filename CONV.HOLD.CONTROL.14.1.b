...
*-----------------------------------------------------------------------------
    $PACKAGE DE.Config
      SUBROUTINE CONV.HOLD.CONTROL.14.1
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.HOLD.CONTROL"           ; * File to be converted
      COMPANY.CODE.POS = 23              ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 9                 ; * Position to add from. (New field number)

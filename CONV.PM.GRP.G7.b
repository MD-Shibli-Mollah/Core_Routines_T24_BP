...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.PM.GRP"                 ; * File to be converted
      YFILE.1 = "F.PM.POSN.REFERENCE"    ; * Additional File to be converted
      COMPANY.CODE.POS = "1"             ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
         ...
         IF MULTI.COMPANY.FILE THEN
            ...
            WHILE K.COMPANY:END.OF.COMPANIES
               READV MNEMONIC FROM F.COMPANY,K.COMPANY,EB.COM.MNEMONIC THEN
                  ...
                  FILE.NAME.1 = 'F':MNEMONIC:'.':FIELD(YFILE.1,'.',2,99)
                  GOSUB MODIFY.FILE
               END
            REPEAT
...
*
      ...
      FOR FILE.TYPE = 1 TO 3
         ...
         YFILE.1 = FILE.NAME.1:SUFFIX
         F.FILE = ""
         F.FILE.1 = ""
         OPEN '',YFILE TO F.FILE THEN
            OPEN '',YFILE.1 TO F.FILE.1 ELSE NULL
            GOSUB MODIFY.FILE.START
         END ELSE
            IF FILE.TYPE = 3 THEN
               OPEN '',YFILE.1 TO F.FILE.1 THEN
                  GOSUB MODIFY.FILE2
               END
            END
         END
...
*
            ...
            LOOP X += 1 UNTIL ADD.FIELD<1,X> = ''
               ...
               NEXT Y
            REPEAT
* Same records with extended keys
            IF LEN(YID) < 3 THEN
               YID1 = YID:YID.X[1,2-(LEN(YID) = 2)]
               WRITE YREC TO F.FILE, YID1
               DELETE F.FILE, YID
            END
*
         END                             ; * Valid Record.
...
*
      ...
      UNTIL END.OF.FILE
*
         READ YREC FROM F.FILE.1, YID ELSE GOTO FATAL.ERROR
         CALL SF.CLEAR(1,7,"CONVERTING RECORD:  ":YID)
         IF YREC<COMPANY.CODE.POS> MATCHES "2A7N" THEN
            TEXT = "CONVERSION ALREADY DONE... ABORT ?"
            CALL OVE
         ...
         END ELSE
*  extended keys
            COUNT.DLR = DCOUNT(YREC<PM.PR.DLR.DSK.GRP>,VM)
            FOR X = 1 TO COUNT.DLR
               YGRP.ID = YREC<PM.PR.DLR.DSK.GRP,X>
               IF YGRP.ID NE '_' THEN
                  IF NOT(NUM(YGRP.ID)) THEN
                     IF LEN(YGRP.ID) < 3 THEN
                        YGRP.ID1 = YGRP.ID:YID.X[1,2-(LEN(YGRP.ID) = 2)]
                        YREC<PM.PR.DLR.DSK.GRP,X> = YGRP.ID1
                     END
                  END
               END
            NEXT X
            WRITE YREC TO F.FILE.1, YID
*
         END                             ; * Valid Record.
*
      REPEAT
...
*
      ...
      ETEXT ="PM.RTN.WHY.PROGRAM.ABORTED"          ; * Used to update F.CONVERSION.PGMS
      CALL PGM.BREAK
*
*************************************************************************
   END

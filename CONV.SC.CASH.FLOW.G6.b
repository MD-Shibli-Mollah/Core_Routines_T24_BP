...
*-----------------------------------------------------------------------------
    $PACKAGE SC.ScvCashAndFundFlow
      SUBROUTINE CONV.SC.CASH.FLOW.G6
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.SC.CASH.FLOW"           ; * File to be converted
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 30                ; * Position to add from. (New field number)
      ADD.FIELD<1,2> = 21
...
*
      ...
      TEXT = ""
*
* must update all 12 files for all months of year
*
      FOR MNTH = 1 TO 12
         IF MNTH < 10 THEN
            MNTH = '0':MNTH
         END
         YFILE = FILE.NAME:MNTH
         F.FILE = ""

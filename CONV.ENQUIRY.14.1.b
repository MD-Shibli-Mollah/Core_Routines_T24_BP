...
*
      ...
      CLS := @(0,4)
      YFILE = "F.ENQUIRY"
      COMPANY.CODE.POS = 47              ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 33                ; * Position to add from. (New field number)
...
*
      ...
      SELECT F.FILE
      PROMPT.USER = 1                    ; * Promp for overrides
      END.OF.FILE = FALSE
      ABORT.FLAG = FALSE
      LOOP
...
*
         ...
         NO.FIELDS = DCOUNT(YREC,FM)
         CALL SF.CLEAR(1,7,"CONVERTING RECORD:  ":YID)
         IF YREC<COMPANY.CODE.POS> MATCHES "2A7N" OR (YREC<COMPANY.CODE.POS> = "" AND YID[1,1] = "%" AND NO.FIELDS GE 50) OR NO.FIELDS GE 50 THEN
            IF PROMPT.USER THEN          ; * Prompt
               YTXT = "ALREADY CONVERTED - CONTINUE / SKIP OVERRIDES / QUIT  (C/S/Q)"
               CALL TXTINP(YTXT, 8, 22, "1.1" , "":FM:"C_S_Q")
               BEGIN CASE
                  CASE COMI = "Q"        ; * Continue no further
                     END.OF.FILE = TRUE
                     ABORT.FLAG = TRUE
                  CASE COMI = "S"        ; * No more prompts
                     PROMPT.USER = ""
               END CASE
            END
         END ELSE

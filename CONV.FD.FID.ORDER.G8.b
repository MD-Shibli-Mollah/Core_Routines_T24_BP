...
*-----------------------------------------------------------------------------
    $PACKAGE FD.Contract
      SUBROUTINE CONV.FD.FID.ORDER.G8
*
*************************************************************************
*
* 29/01/98 - GB9800085
*            Must use correct field numbers
*
* 19/09/02 - GLOBUS_EN_10001176
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.FD.FID.ORDER"           ; * File to be converted
...
*
               ...
               REPEAT
*
               IF YREC<69> = "FWD" AND YREC<40> NE '' THEN
*
* Must have charges on contract
*
                  YREC<38,1> = YREC<8>   ; * VALUE DATE
                  YREC<39,1> = "DRAWDOWN"
                  YREC<40> = LOWER(YREC<40>)
                  YREC<41> = LOWER(YREC<41>)
                  YREC<42> = LOWER(YREC<42>)
               END ELSE
                  YREC<38> = ""
                  YREC<39> = ""
                  YREC<40> = ""
               ...
               END
*
               IF YREC<69> = "FWD" AND YREC<48> NE '' THEN
*
* Must have tax on contract
*
                  YREC<47> = YREC<8>     ; * VALUE DATE
                  YREC<48> = LOWER(YREC<48>)
                  YREC<49> = LOWER(YREC<49>)
                  YREC<50> = LOWER(YREC<50>)
                  YREC<51> = LOWER(YREC<51>)
               END ELSE
                  YREC<47> = ""
                  YREC<48> = ""
                  YREC<49> = ""
...
*
      ...
      ETEXT ="FD.RTN.WHY.PROGRAM.ABORTED.1"        ; * Used to update F.CONVERSION.PGMS
      CALL PGM.BREAK
*
*************************************************************************
   END

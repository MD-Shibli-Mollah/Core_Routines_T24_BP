* @ValidationCode : MjotNjA5NzEyMTIwOkNwMTI1MjoxNjA0NTc2MDY4MzM4OnNrYXlhbHZpemhpOjU6MDowOjE6ZmFsc2U6Ti9BOkRFVl8yMDIwMTAuMjAyMDA5MTktMDQ1OTo2NDo2NA==
* @ValidationInfo : Timestamp         : 05 Nov 2020 17:04:28
* @ValidationInfo : Encoding          : Cp1252
* @ValidationInfo : User Name         : skayalvizhi
* @ValidationInfo : Nb tests success  : 5
* @ValidationInfo : Nb tests failure  : 0
* @ValidationInfo : Rating            : N/A
* @ValidationInfo : Coverage          : 64/64 (100.0%)
...
SUBROUTINE PPEWSP.GENERATE.UNIQUE.REFERENCE(iLockingId,iAgentDigits,iRandomDigitsLen,iReserved,oUniqueReference,oReserved)
*-------------------------------------------------------------------------------------------------------------------------------------
...
*So, Changing Read to Readu to lock the record
    ...
    IF rLocking EQ '' THEN  ;* if there is no record, write a new one with the sequence number and today date
        ...
        rLocking<2> = SeqNo
    END ELSE
        IF rLocking<2> EQ '999999' THEN  ;* If value reaches 999999 then start the sequence again
            RSuffix = ''
            SeqNo = '1' ;* sequence number
            rLocking<2> = SeqNo
        END ELSE

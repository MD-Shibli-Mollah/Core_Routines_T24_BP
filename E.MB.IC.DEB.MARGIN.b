...
*
*Subroutine to get the debit margin for Debit Interest Calculation
*-----------------------------------------------------------------------------
* <Rating>338</Rating>
*-----------------------------------------------------------------------------
    $PACKAGE IC.ModelBank
    SUBROUTINE E.MB.IC.DEB.MARGIN
*
...
********
    ...
    DR.RC.DATES = RAISE(RATE.CHANGE.DATES<2>)
    ...
    CNT11 = ""
    CNT11 = 403
    tmp=EB.Reports.getRRecord(); tmp<403>=""; EB.Reports.setRRecord(tmp)

    FOR RATE.CNT = 1 TO NO.OF.RATES
        IF OUT.DATA<1,RATE.CNT> EQ '' THEN
            tmp=EB.Reports.getRRecord(); tmp<403,-1>=0; EB.Reports.setRRecord(tmp)
        END ELSE
            tmp=EB.Reports.getRRecord(); tmp<403,-1>=OUT.DATA<1,RATE.CNT>; EB.Reports.setRRecord(tmp)
        END
...
***                *OUT.DATA<CNT> := " from 0 up to ":FMT(RET.DATA<IC.RC.DEBIT.BAND.UPTO, I, 1>,'L,0')
                ...
                FOR J = 2 TO NO.OF.RATES
                    IF RET.DATA<IC.RC.DEBIT.BAND.RATE, I, J> NE '' THEN
                        ...
                        CNT += 1
                        OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.DEBIT.BAND.RATE, I, J>,'$',3)

***                    OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.DEBIT.BAND.RATE, I, J>,'$',3):"%":" over "
            ...
            IF NO.OF.RATES GT 1 THEN
                FOR J = 1 TO NO.OF.RATES
                    ...
                    IF J NE NO.OF.RATES THEN
                        ...
                        OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.DEBIT.BAND.RATE, I, J>,'$',3)
***                   OUT.DATA<CNT> = FIELD(RET.DATA<IC.RC.DEBIT.BAND.RATE, I, J>,'$',3):"%":" up to "
...
***  OUT.DATA<CNT> := FIELD(RET.DATA<IC.RC.DEBIT.BAND.RATE, I, 1>,'$',3):"% over 0"

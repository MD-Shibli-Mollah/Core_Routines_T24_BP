...
*-----------------------------------------------------------------------------
    ...
    SUBROUTINE CONV.TRN.CON.TRADE.DATE
*
*
*********************************************************
*
...
* CI_10028622 E
        IF R.COMPANY(EB.COM.MNEMONIC) EQ R.COMPANY(EB.COM.FINAN.FINAN.MNE) THEN
            GOSUB OPEN.FILES
            GOSUB PROCESS.TRN.CON.TRADE.DATE
        END
* CI_10028622 S
...
*
    FN.TRN.CON.TRADE.DATE = 'F.TRN.CON.TRADE.DATE'
    F.TRN.CON.TRADE.DATE = ''
    CALL OPF(FN.TRN.CON.TRADE.DATE,F.TRN.CON.TRADE.DATE)
*
    RETURN

******************************************************************
PROCESS.TRN.CON.TRADE.DATE:
*
* Select list
*
        SP.LIST = ''
        SELECT.SP = 'SSELECT ':FN.TRN.CON.TRADE.DATE
        CALL EB.READLIST(SELECT.SP,SP.LIST,'','','')

*
* Main loop (process TRN.CON.TRADE.DATE)
*
        LOOP
...
*
        WHILE SP.CODE DO
*
            CALL F.READ(FN.TRN.CON.TRADE.DATE,SP.CODE,R.FILE,F.TRN.CON.TRADE.DATE,ER)
            IF ER THEN
                E = 'RECORD & NOT FOUND ON FILE & ':FM:SP.CODE:VM:"F.TRN.CON.TRADE.DATE"
                GOTO FATAL.ERROR
            END
*
            SP.CODE.NEW = SP.CODE:'.'

            CALL F.WRITE(FN.TRN.CON.TRADE.DATE,SP.CODE.NEW,R.FILE)    ;*save new record
*
            CALL F.DELETE(FN.TRN.CON.TRADE.DATE,SP.CODE)    ;*delete old record
*
...
FATAL.ERROR:
    ...
    TEXT = E
    CALL FATAL.ERROR('CONV.TRN.CON.TRADE.DATE')

...
*-----------------------------------------------------------------------------
    $PACKAGE SC.SctOffMarketTrades
      SUBROUTINE CONV.SECURITY.TRANSFER.G11B
*
...
*23/08/00 - GB0002052
*           jBASE changes.
*           Convert key words used as variable in these programs.
*************************************************************************
INITIALISE:
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.SECURITY.TRANSFER"      ; * File to be converted
      ORIGINAL.FILE = YFILE              ; * Store this it will changed
      COMPANY.CODE.POS = "79"            ; * Position of new XX.CO.CODE in the file
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = "60"              ; * Position to add from. (New field number)

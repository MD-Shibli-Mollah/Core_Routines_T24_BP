* @ValidationCode : MjotMTYyOTQ0MzI1NzpDcDEyNTI6MTQ4NTk0MTA4MTY2MzpkcG9vcm5pbWE6MjowOjA6LTE6ZmFsc2U6Ti9BOkRFVl8yMDE3MDEuMDo1Mjo1Mg==
...
$PACKAGE SC.ScvReports
    SUBROUTINE E.SC.CALC.PERF.CONTR
************************************************************
*
...
*
    ...
    LAST.CONTR.POS = DCOUNT(R.SEC.ACC.MASTER<SC.ScoPortfolioMaintenance.SecAccMaster.ScSamPerformDate>,@VM)
*
...
*
    COUNT.PERFORMS = DCOUNT(PERFORM.DATES,@VM)
...
*
    ...
    IF CCY1 NE CCY2 AND CONTRIBUTIONS THEN
        VAL1 = CONTRIBUTIONS
        VAL2 = ''
        RET.CODE = ''
        RATE = ''
...
EB.SystemTables.setEtext(tmp.ETEXT)
        ...
        END
    END  ELSE
        EB.Reports.setOData(CONTRIBUTIONS)
    END
*
    RETURN
    END

* @ValidationCode : MjotMTIzMDQ1OTA2MzpDcDEyNTI6MTQ4OTY0NDY5Mzc0MzpoYXJyc2hlZXR0Z3I6LTE6LTE6MDotMTpmYWxzZTpOL0E6REVWXzIwMTcwMi4wOi0xOi0x
...
*-----------------------------------------------------------------------------
    ...
    SUBROUTINE OC.UPD.FIXED.RATE.LEG1(APPL.ID,APPL.REC,FIELD.POS,RET.VAL)
*-----------------------------------------------------------------------------
*<Routine description>
*
*
* Attached as a link routine in TX.TXN.BASE.MAPPING record to
* report the fixed rate on the leg1 of the contract.
*
*
* Incoming parameters:
*
...
* Outgoing parameters:
*
* RET.VAL	-	1. If Fixed Rate on Asset Leg, Leg1 value to be updated with SW.AS.FIXED.RATE
*               2. If Fixed Rate on Liab Leg, Leg1 value to be updated with SW.LB.FIXED.RATE
*               3. If Fix/Fix Interest Rate/Currency Swap, then leave blank
*               4. In case of Flt/Flt identify based on duration, spread
*                   4.a Diff duration, then spread of leg which is shorter duration
*                   4.b Same duration and spread on one leg, then the spread value.
*                   4.c Same duration and spread on both leg, then blank
*                   4.d Same duration and spread not present in both the leg, then blank
*
*-----------------------------------------------------------------------------
        ...
        CASE APPL.ID[1,2] EQ 'SW'
            ...
            BEGIN CASE
                ...
                CASE assetFixedRate AND NOT(liabFixedRate)
                    RET.VAL = APPL.REC<SW.Contract.Swap.AsFixedRate>
...
*** <desc> </desc>
    ...
    BEGIN CASE
        ...
        CASE assetSpread AND NOT(liabSpread)
            ...
            RET.VAL = APPL.REC<SW.Contract.Swap.AsSpread>   ;*T24 bank receives spread.

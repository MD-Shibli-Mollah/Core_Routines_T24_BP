* @ValidationCode : Mjo5OTI5MjkzNTU6Q3AxMjUyOjE2MTAzNzM5MzE4NjM6cmRlZXBpZ2E6NTowOjA6MTpmYWxzZTpOL0E6REVWXzIwMjAxMi4yMDIwMTEyOC0wNjMwOjQyOjQx
* @ValidationInfo : Timestamp         : 11 Jan 2021 19:35:31
...
SUBROUTINE SCDX.TRS.UPD.BUY.DEC.MKR.DOB(TXN.ID,TXN.REC,TXN.DATA,RET.VAL)
*-----------------------------------------------------------------------------
...
* Outgoing parameters:
**********************
* RET.VAL  -  If Customer is Buyer, then RET.VAL will be Date of Birth of Decision Maker
*
*-----------------------------------------------------------------------------
...
*** <desc>Process to determine the customer side is Buyer when Decision Maker inputted in txn for reporting purpose </desc>
    ...
    BEGIN CASE
* Security Txn:
* If the customer mentioned in CUSTOMER.NO is Buyer, then Date of Birth of the customer defined in the field DECISION.MKR.ID will be returned
...
* Derivative Txn:
* If PRI.CUST.TYPE is Customer and PRI.BUY.SELL is BUY, then shall populate this field with the Date of Birth of the customer defined in the field DECISION.MKR.ID.
* If SEC.CUST.TYPE is Customer and SEC.BUY.SELL is BUY, then shall populate this field with the Date of Birth of the customer defined in the field DECISION.MKR.ID.

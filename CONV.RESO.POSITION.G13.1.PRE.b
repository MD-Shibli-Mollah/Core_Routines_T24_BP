...
*-----------------------------------------------------------------------------
    $PACKAGE RP.Contract
      SUBROUTINE CONV.RESO.POSITION.G13.1.PRE
*-----------------------------------------------------------------------------
* A new lookup key has been created on RESO.POSITION. To update this field all REPO's that have not
* matured need to be selected & then update the RESO.POSITION.
...
* This will select all records which have not matured & are DEPOSIT's (REPO)
      ...
      COMMAND = 'SELECT ':FN.REPO:' WITH MATURITY.DATE >= ':TODAY:' AND WITH TRANSACTION.INDEX = "LOAN"'
      REPO.LIST = ''
      SELECTED = ''
      SRC = ''
...
*-----------------------------------------------------------------------------
CREATE.POSITION.KEY:
* This will create the key that can be used to update the RESO.POSITION file.

...
* Version 14 15/05/01 GLOBUS Release No. G12.1.00 27/07/01
*-----------------------------------------------------------------------------
* <Rating>1455</Rating>
*-----------------------------------------------------------------------------
    $PACKAGE EB.OverrideProcessing
      SUBROUTINE CONV.DISPO.ITEMS.G12.1.00
*
...
*
** 27/07/01 - GLOBUS_EN_10000035
*             CUSTOMER.NO (2) has switched places with GEN.COMMENT (11)
*             Build new record ID.    TransactionId * CompanyMnemonic
*  30/07/01   Internal subroutine:  FATAL.ERROR
...
*  03/12/01   GLOBUS_BG_100000280 /S
*             FIX: Conversion routine is only creating F.DISPO.ITEMS records
*             using Company Mnemonic of company the conversion is being performed
*             in.  Needs to use Company Mnemonic of each company for multi-company
*             environments.
*
* 23/08/02 - GLOBUS_EN_10000971
*          Conversion Of all Error Messages to Error Codes
*
* 05/12/02 - GLOBUS_CI_10005261
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.DISPO.ITEMS"            ; * File to be converted
      ...
      READ R.PGM.FILE FROM F.PGM.FILE,APPLICATION ELSE
         ID = APPLICATION
         YFILE = 'F.PGM.FILE'
         ETEXT = "EB.RTN.MISS.FILE":FM:APPLICATION:VM:YFILE  ; * GLOBUS_EN_10000035
         GOTO FATAL.ERROR
      END
      ...
      READ R.FILE.CONTROL FROM F.FILE.CONTROL,ID ELSE
         ...
         ETEXT = "EB.RTN.MISS.FILE.1":FM:ID:VM:YFILE         ; * GLOBUS_EN_10000035
         GOTO FATAL.ERROR
      END
      R.FILE.CONTROL<EB.FILE.CONTROL.CLASS> = "FIN"          ; * Company Level file
...
* Create environment level files for F.DISPO.OFFICER
      ETEXT = ""
      CALL EBS.CREATE.FILE("DISPO.ITEMS","",ETEXT)
...
*
      GOSUB PRINT.SUMMARY
* GLOBUS_CI_10005261  /S
*      PRINT
*      TEXT = 'CONVERSION COMPLETE'
*      CALL REM
* GLOBUS_CI_10005261  /E
...
* Open file to transfer data into
         DEST.FILE = "F.DISPO.ITEMS" : SUFFIX

         F.DEST.FILE = ""
         OPEN DEST.FILE TO F.DEST.FILE ELSE
            ETEXT ="EB.RTN.UNABLE.OPEN.FILE":FM: DEST.FILE
            GOTO FATAL.ERROR
         END
...
*
            READ YREC FROM F.FILE, YID ELSE
               ETEXT = "EB.RTN.MISS.FILE.2":FM:YID:VM:YFILE  ; * GLOBUS_EN_10000035
               GOTO FATAL.ERROR
            END
...
* GLOBUS_EN_10000035 /S
* Store contents of fields before they are switched.
               FLD.GEN.COMMENTS = YREC<11>
               FLD.CUSTOMER.NO = YREC<2>

* Prepare GEN.COMMENTS and CUSTOMER.NO fields
               YREC<2> = ""
               YREC<11> = ""

* Transfer contents of field 2 into field 11
               YREC<11> = FLD.CUSTOMER.NO

* Merge all COMMENTS fields together to form new GEN.COMMENTS field 2.
               N.VM = DCOUNT(FLD.GEN.COMMENTS,VM)
               FOR VPOS = 1 TO N.VM
                  N.SM = DCOUNT(FLD.GEN.COMMENTS<1,VPOS>,SM)
                  FOR SPOS = 1 TO N.SM
                     ST = FLD.GEN.COMMENTS<1,VPOS,SPOS>
                     IF YREC<2> NE "" THEN
                        YREC<2> = SM : ST
                     END ELSE
                        YREC<2> = ST
...
*
* GLOBUS_EN_10000035 /S
* Check SUFFIX.  SUFFIX is $HIS then format of ID that needs to be written is different:
* Instead of "TransactionID;Number", the ID needs to be set to
* "TransactionId*CompanyMnemonic;Number"
* Otherwise call CALL EB.SET.DISPO.ID to add the CompanyMnemonic suffix.

               BEGIN CASE
                  CASE SUFFIX = "$HIS"
                     TRANS.ID = FIELD(YID,";",1)
                     HIST.NO = FIELD(YID,";",2)
                     YID = TRANS.ID : "*" : MNEMONIC : ";" : HIST.NO
                  CASE 1
* GLOBUS_BG_100000280 /S
* FIX: Conversion routine is only creating F.DISPO.ITEMS records
* using Company Mnemonic of company the conversion is being performed
* in.  Needs to use Company Mnemonic of each company for multi-company
* environments.
                     YID = YID : "*" : MNEMONIC
* GLOBUS_BG_100000280 /E
               END CASE

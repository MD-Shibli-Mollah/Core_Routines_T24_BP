...
*-----------------------------------------------------------------------------
    $PACKAGE SC.SccClassicCA
      SUBROUTINE CONV.REDEMPTION.CUS.G6.1
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.REDEMPTION.CUS"         ; * File to be converted
      COMPANY.CODE.POS = "63"            ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
** file is being converted more than once.
      ADD.FIELD = ''
      ADD.FIELD<1,1> = "40"              ; * Position to add from. (New field number)
      ADD.FIELD<2,1> = "15"              ; * Number of fields to add.
      ADD.FIELD<1,2> = "11"              ; * Position to add from. (New field number)

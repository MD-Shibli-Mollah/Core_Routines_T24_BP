...
* Version 4 22/05/01  GLOBUS Release No. 200511 31/10/05
*-----------------------------------------------------------------------------
* <Rating>901</Rating>
*-----------------------------------------------------------------------------
    $PACKAGE SC.ScoPortfolioMaintenance
      SUBROUTINE CONV.SEC.ACC.MASTER.14.1
*
...
*
      ADD.FIELD = ''
      ADD.FIELD<1,1> = 62                ; * Position to add from. (New field number)
      ADD.FIELD<2,1> = 5                 ; * Number of fields to add.
      ADD.FIELD<1,2> = 71                ; * Position to add from. (New field number)
...
! The following to be done only for BDG at 14.1.0
            ...
            IF MNEMONIC = 'BDG' THEN
               ...
               IF YLOCAL.REF = '' THEN YREC<70> = ''
            END
! End of BDG specific code
*

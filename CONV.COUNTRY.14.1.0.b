...
*-----------------------------------------------------------------------------
    $PACKAGE EB.SystemTables
      SUBROUTINE CONV.COUNTRY.14.1.0
*
...
*
      ...
      CLS := @(0,4)
      YFILE = "F.COUNTRY"
      COMPANY.CODE.POS = 17              ; * Position of new XX.CO.CODE in the file
      F.PGM.FILE = ''
...
*
            ...
            IF MNEMONIC = 'BDG' THEN
               YREC<EB.COU.PRESENTATION.CODE> = YREC<EB.COU.LOCAL.REF,4>
               YREC<EB.COU.LOCAL.REF,4> = ''
               YLOCAL.REF = YREC<EB.COU.LOCAL.REF>
               CONVERT VM TO '' IN YLOCAL.REF
               IF YLOCAL.REF = '' THEN YREC<EB.COU.LOCAL.REF> = ''
            END
*

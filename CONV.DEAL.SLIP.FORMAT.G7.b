...
*-----------------------------------------------------------------------------
    $PACKAGE EB.Dealslip
      SUBROUTINE CONV.DEAL.SLIP.FORMAT.G7
*
...
*
      ...
      NEXT X
      CLS := @(0,4)
      YFILE = "F.DEAL.SLIP.FORMAT"       ; * File to be converted
...
*
            ...
            REPEAT
*
            IN.FILE.NAME = YREC<13,1>["_",1,1]
            IND = 1
            COMP.FIELD = YREC<13>
            LOOP
            WHILE COMP.FIELD<1,IND>
               FIELD.NO = COMP.FIELD<1,IND>["_",3,1]
               IF NUM(FIELD.NO) THEN
                  SS.REC = ""
                  CALL GET.STANDARD.SELECTION.DETS(IN.FILE.NAME, SS.REC)
                  LOCATE FIELD.NO IN SS.REC<3,1> SETTING FN.POS THEN
                     COMP.FIELD<1,IND> = COMP.FIELD<1,IND>["_",1,2]:"_":SS.REC<1,FN.POS>:"_":COMP.FIELD<1,IND>["_",4,99]
                  END
               END
               IND += 1
            REPEAT
            YREC<13> = COMP.FIELD
*
            COMP.FIELD = YREC<14>        ; * compiled field
            IND = 1
            LOOP
            WHILE COMP.FIELD<1,IND>
               FILE.NO = COMP.FIELD<1,IND>["_",1,1]
               IF FILE.NO THEN
                  FIELD.NO = COMP.FIELD<1,IND>["_",2,1]
                  IF NUM(FIELD.NO) THEN
                     IN.FILE.NAME = YREC<13,FILE.NO>["_",1,1]
                     SS.REC = ""
                     CALL GET.STANDARD.SELECTION.DETS(IN.FILE.NAME, SS.REC)
                     LOCATE FIELD.NO IN SS.REC<3,1> SETTING FN.POS THEN
                        COMP.FIELD<1,IND> = COMP.FIELD<1,IND>["_",1,1]:"_":SS.REC<1,FN.POS>:"_":COMP.FIELD<1,IND>["_",3,99]
                     END
                  END
               END
               IND += 1

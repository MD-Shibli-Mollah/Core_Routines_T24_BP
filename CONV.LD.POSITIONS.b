...
*-----------------------------------------------------------------------------
    $PACKAGE LD.Foundation
      SUBROUTINE CONV.LD.POSITIONS (CONVERT.COMPANY, R.COMP)
*
*=======================================================================
...
*-----------------------------------------------------------------------
      ...
      IF R.COMP<EB.COM.CONSOLIDATION.MARK> NE 'C' AND R.COMP<EB.COM.CONSOLIDATION.MARK> NE 'R' THEN
         LOCATE 'LD' IN R.COMP<EB.COM.APPLICATIONS,1> SETTING POS ELSE POS = ''
      END
      IF NOT(POS) THEN
...
*
*
      SELECT.COMMAND = "SELECT ":CAT.ENT.TODAY.FILE:" WITH SYSTEM.ID LIKE 'LD...' BY SEL.CCY"
      CAT.ENT.TODAY.KEYS = ""
      CALL EB.READLIST(SELECT.COMMAND, CAT.ENT.TODAY.KEYS, "LD.POSITIONS", "", "")
*
...
*
         READ CATEG.ENTRY.REC FROM F.CATEG.ENTRY,YENT.ID ELSE
            ...
            MODULE.IN.ERROR = "LD.EOD.UPDATE.POSITIONS"

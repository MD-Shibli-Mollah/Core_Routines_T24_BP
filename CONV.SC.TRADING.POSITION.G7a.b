...
* Version 7 07/06/01  GLOBUS Release No. 200511 31/10/05
*-----------------------------------------------------------------------------
* <Rating>774</Rating>
*-----------------------------------------------------------------------------
    $PACKAGE SC.SctDealerBookPosition
      SUBROUTINE CONV.SC.TRADING.POSITION.G7a
*
...
*
      ...
      END
*
* no conversion if previous release GE G6.1.11
*
      PREVIOUS.RELEASE = R.SPF<SPF.PREVIOUS.RELEASE>
      IF PREVIOUS.RELEASE[1,5] EQ 'G6.1.' THEN
         VER.NO = PREVIOUS.RELEASE['.',3,1]
         IF VER.NO GE 11 THEN
...
** file is being converted more than once.
      CANCEL.FIELD = ""
      CANCEL.FIELD<1,1> = 30             ; * Position to cancel from.
      CANCEL.FIELD<2,1> = 3              ; * Number of fields to cancel.
      CANCEL.FIELD<1,2> = 25             ; * Position to cancel from.
      CANCEL.FIELD<2,2> = 2              ; * Number of fields to cancel.
*
...
** file is being converted more than once.
      ...
      ADD.FIELD<1,1> = 25
      ADD.FIELD<2,1> = 3
      ADD.FIELD<1,2> = 24                ; * Position to add from. (New field number)

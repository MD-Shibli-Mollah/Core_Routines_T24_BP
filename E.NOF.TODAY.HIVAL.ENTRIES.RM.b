...
*-----------------------------------------------------------------------------
    ...
    SUBROUTINE E.NOF.TODAY.HIVAL.ENTRIES.RM(OUT.ARRAY)
*-----------------------------------------------------------------------------
...
*             in descending order by  local currency amount and to show transactions
*             done for the customers pertaining to the Relationship Manager
*
*-----------------------------------------------------------------------------
...
***<desc> Process and pass the out.array </desc>
    ...
    WHILE CR.NO
        ...
        GOSUB PROCESS.CUST.ACC ; *Check whether the customer account officer is same as the relationship manager Or Whether matches with the one given in criteria
...
***<desc> To get the Selection criteria </desc>
    ...
    LOCATE 'ACCOUNT.OFFICER' IN EB.Reports.getDFields()<1> SETTING POS1 THEN
    ACCT.OFFICER = EB.Reports.getDRangeAndValue()<POS1>
    OPERAND.ACC = EB.Reports.getDLogicalOperands()<POS1>
    END ELSE
    ACCT.OFFICER = EB.SystemTables.getRUser()<EB.Security.User.UseDepartmentCode>
    OPERAND.ACC = '1'
    END
...
*** <region name= PROCESS.CUST.ACC>
PROCESS.CUST.ACC:
*** <desc>Check whether the customer account officer is same as the relationship manager Or Whether matches with the one given in criteria </desc>
    CUS.NO = ''  ; CUS.ACC.OFF = '' ; CUS.OUT = ''
    CUS.NO = R.CR.CLOG<CR.Analytical.ContactLog.ContLogContactClient>

    R.CUSTOMER = '' ; CUS.ERR = ''
    R.CUSTOMER = ST.Customer.Customer.Read(CUS.NO, CUS.ERR)

    IF NOT(CUS.ERR) THEN
        CUS.ACC.OFF = R.CUSTOMER<ST.Customer.Customer.EbCusAccountOfficer>
        CR.ModelBank.SetCondFlag(CUS.ACC.OFF,ACCT.OFFICER,OPERAND.ACC,CUS.OUT)
    END
    RETURN
*** </region>
